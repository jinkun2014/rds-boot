...
<#if EntityInfo.primaryKey??>
    ...
    @Override
    public ${EntityInfo.entityName} findFirstByForm(${EntityInfo.entityName}Form form) {
        ${EntityInfo.entityName}Example example = new ${EntityInfo.entityName}Example();
        //设置分页
        example.setStart(0L);
        example.setSize(1L);
    ...
    public RespResult<${EntityInfo.entityName}Result> update(${EntityInfo.primaryKey.type} ${EntityInfo.primaryKey.name},${EntityInfo.entityName}Form form) {
        ...
        Date now = new Date();
        entity.setGmtModified(now);
        ${EntityInfo.entityName ? uncap_first}Mapper.updateByPrimaryKeySelective(entity);
    ...
    public RespResult<${EntityInfo.entityName}Result> delete(${EntityInfo.primaryKey.type} ${EntityInfo.primaryKey.name}) {
        ${EntityInfo.entityName} ${EntityInfo.entityName ? uncap_first} = ${EntityInfo.entityName ? uncap_first}Mapper.selectByPrimaryKey(${EntityInfo.primaryKey.name});
        if (${EntityInfo.entityName ? uncap_first} == null) {
            return RespResult.fail("删除失败,${EntityInfo.primaryKey.name}:" + ${EntityInfo.primaryKey.name} + " 的记录不存在");
        }
        ${EntityInfo.entityName ? uncap_first}.setDelete(true);
        ${EntityInfo.entityName ? uncap_first}.setGmtModified(new Date());
        ${EntityInfo.entityName ? uncap_first}Mapper.updateByPrimaryKey(${EntityInfo.entityName ? uncap_first});
    ...
    public RespResult<${EntityInfo.entityName}Result> deleteByIds(String ids) {
        ...
        ${EntityInfo.entityName ? uncap_first}.setDelete(true);
        ${EntityInfo.entityName ? uncap_first}.setGmtModified(new Date());
        ${EntityInfo.entityName ? uncap_first}Mapper.updateByExampleSelective(${EntityInfo.entityName ? uncap_first}, example);
        return RespResult.ok("删除成功");
    }
